...
apply_optimalRF <- function(i, key, columns_set_up, optimal_ranger) {
  ...
  print(station_no)
  test_data <- read.csv(paste0('../data/allpredictors/allpredictors_', station_no, '.csv'))
  ...
  rf.result <- test_data_pre %>%
    ...
    mutate(average_dis = rowMeans(select(., c("pcr", "lis", "wg3")), na.rm = TRUE)) %>% 
  
    mutate(dis_corrected = replace(dis_corrected, dis_corrected < 0, 0)) %>%
    mutate(res = obs - average_dis) %>%  # Calculate residuals based on average_dis
  ...
  rf.eval <- rf.result %>%
    summarise(grdc_no = station_no,
              KGE = KGE(sim = average_dis, obs = obs,
                        s = c(1, 1, 1), na.rm = TRUE, method = "2009"), # simulated discharge is set to average_dis. This is the unclaibrated 
